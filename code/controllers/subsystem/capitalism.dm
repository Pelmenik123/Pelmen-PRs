...
/datum/controller/subsystem/capitalism/proc/payment_process()
	...
	for(var/datum/money_account/account in GLOB.all_money_accounts)
		if(account.salary_payment_active && account.linked_job.salary && !account.suspended)
			if(payment_account.charge(round(account.linked_job.salary / 2), account, "Выплата зарплаты персоналу.", "Nanotrasen personal departament" , "Поступление зарплаты.", "Поступление зарплаты" ,"Biesel TCD Terminal #[rand(111,333)]"))
				account.notify_pda_owner("<b>Поступление зарплаты </b>\"На ваш привязанный аккаунт поступило [round(account.linked_job.salary / 2)] кредитов\" (Невозможно Ответить)", FALSE)
				total_salary_payment += round(account.linked_job.salary / 2)
			else
				return FALSE
